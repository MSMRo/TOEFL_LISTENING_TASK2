...
# ---- Silence generator: works with any pydub version ----
try:
    from pydub.generators import Silence
    def make_silence(duration_ms: int) -> AudioSegment:
        return Silence(duration=duration_ms).to_audio_segment()
except Exception:
    # Older pydub: use built-in silent segment
    def make_silence(duration_ms: int) -> AudioSegment:
        return AudioSegment.silent(duration=duration_ms)

# Note: pyttsx3 is not fully thread-safe; create/stop inside functions.
import pyttsx3
...
def list_system_voices() -> List[Dict]:
    """List OS voices via pyttsx3 (cached per session)."""
    engine = pyttsx3.init()
...
def parse_dialogue(text: str) -> List[Tuple[str, str]]:
    """
    Parse lines of the form 'Role: text'. Lines without ':' are ignored.
    """
    segments = []
...
def extract_pause_chunks(text: str) -> List[Tuple[str, Optional[int]]]:
    """
    Split text into [(chunk_text, pause_ms_after_chunk)] using [pause=###ms] tags.
    Example: "Hello [pause=300ms] world" -> [("Hello", 300), ("world", None)]
    """
    pos = 0
...
def pick_auto_voice_for_role(role: str, voices_all: List[Dict], used_ids: set) -> Optional[str]:
    """
    Simple heuristic to auto-assign voices:
    - 'prof' → prefer male voices; 'student' → prefer female voices (if available)
    - avoid reusing voices when possible
    """
    rlow = role.lower()
    candidates = voices_all[:]

    def contains_key(v: Dict, key: str) -> bool:
        return (key in (v.get("name") or "").lower()) or \
               (key in (v.get("id") or "").lower()) or \
               (key in (v.get("gender") or "").lower())

    preferred = []
    if "prof" in rlow:
        preferred = [v for v in candidates if contains_key(v, "male")]
    elif "student" in rlow:
        preferred = [v for v in candidates if contains_key(v, "female")]

    preferred = [v for v in preferred if v["id"] not in used_ids]
    if preferred:
...
def tts_to_wav_bytes(text: str, voice_id: str, rate_wpm: int, volume: float) -> bytes:
    """Synthesize a single utterance to WAV bytes using pyttsx3."""
    engine = pyttsx3.init()
...
def synthesize_dialogue(
    ...
    """
    Synthesizes the full dialogue with per-role voices, supports inline [pause=XXXms].
    Returns WAV bytes.
    """
    ...
    for idx, (role, text) in enumerate(segments):
        ...
        # split by inline pause tags to add internal silences
...
with st.expander("ℹ️ How it works", expanded=False):
    st.markdown(
        """
- Paste a dialogue using one line per turn with format **`Role: text`**.
- Roles are auto-detected; the app auto-assigns distinct voices (you can tweak them).
- Use inline **pauses** with tags like `[pause=300ms]`.
- Click **Read now** to synthesize & play immediately (offline).
...
with right:
    ...
    if not voices_all:
        st.error("No OS voices found. Install voices or test on another machine.")
    else:
...
# Auto-assign or maintain mapping
...
if read_now:
    if not segments:
        ...
        try:
            ...
            st.error(
                "Export error. Make sure **ffmpeg** is installed and in your PATH.\n"
                f"Details: {e}"
            )
...
- **Prosody**: Inline `[pause=XXXms]` tags help with timing. For natural prosody, consider SSML (cloud TTS).
- **Scaling**: For long scripts or many roles, an API TTS with multi-voice SSML is more robust.
        """
    )
